...
static void DecodeDJIMotor(CANInstance *_instance)
{
    ...
    measure->speed_rmp = (int16_t)(rxbuff[2] << 8 | rxbuff[3]);
...
DJIMotorInstance *DJIMotorInit(Motor_Init_Config_s *config)
{
   ...
   //instance->motor_controller.smc = config->controller_param_init_config.smc;
   //SMCInit(&config->controller_param_init_config.smc);
...
void DJIMotorSetPower(DJIMotorInstance *motor, float power,uint8_t power_flag)
{
    motor->measure.vt_power = power;
    motor->measure.power_flag = power_flag;
}
...
void DJIMotorSetSMCRef(DJIMotorInstance *motor, float ref)
{
    motor->motor_controller.smc_ref = ref;
}
void DJIMotorSetSMCdifferent(DJIMotorInstance *motor, float ref)
{
    motor->motor_controller.smc_diff = -ref;
}
...
static float smc_sat(float x)
{
    ...
    else                return x;  // 边界层内线性，外为符号函数
}
float sign_smooth(float x,float eps)
{
    return x/(fabsf(x)+eps);
}
float SMC_Calc_1(SMCInstance *smc,float actual_val,float ref)
{
    ...
    //   }
    // 步骤1：计算实时偏差
   smc->smc_e = ref-actual_val;
    smc->smc_s = smc->smc_e + SMC_LAMBDA_1 * smc->smc_e_int;
    smc->smc_eq = SMC_K_1 * smc->smc_s; // 符号函数
    smc->smc_sw = SMC_beta_1 *  sign_smooth(smc->smc_s,SMC_alpha_1); 
    // 步骤2：计算滑模面S = e + c*∫e dt（积分项消除静差，离散化计算）
    smc->smc_e_int+= smc->smc_e * SMC_T;  // 积分离散化：∫e dt ≈ Σe*T
   
    smc->smc_u=smc->smc_eq+smc->smc_sw;
    
    // 步骤4：控制量限幅（根据实际执行器能力，如PWM占空比0~100，电压0~24V）
    if(smc->smc_u > 10000.0f) smc->smc_u = 10000.0f;
    if(smc->smc_u < -10000.0f)   smc->smc_u =- 10000.0f;
    
    return smc->smc_u;
}

float SMC_Calc_2(SMCInstance *smc,float actual_val,float ref)
{
    ...
    // 步骤1：计算实时偏差
   smc->smc_e =  ref-actual_val;
    smc->smc_s = smc->smc_e + SMC_LAMBDA_2 * smc->smc_e_int;
    smc->smc_eq = SMC_K_2 * smc->smc_s; // 符号函数
    smc->smc_sw = SMC_beta_2 *  sign_smooth(smc->smc_s,SMC_alpha_2); 
    // 步骤2：计算滑模面S = e + c*∫e dt（积分项消除静差，离散化计算）
    smc->smc_e_int+= smc->smc_e * SMC_T;  // 积分离散化：∫e dt ≈ Σe*T
...
float SMC_Calc_d(SMCInstance *smc,float actual_val,float ref)
{
    //  static uint8_t count = 0;
    // if(count==0)
    //   {
    ...
    // 步骤1：计算实时偏差
   smc->smc_e = ref-actual_val;
    smc->smc_s = smc->smc_e + SMC_LAMBDA_d * smc->smc_e_int;
    smc->smc_eq = SMC_K_d * smc->smc_s; // 符号函数
    smc->smc_sw = SMC_beta_d *  sign_smooth(smc->smc_s,SMC_alpha_d); 
    // 步骤2：计算滑模面S = e + c*∫e dt（积分项消除静差，离散化计算）
    smc->smc_e_int+= smc->smc_e * SMC_T;  // 积分离散化：∫e dt ≈ Σe*T
...
void DJIMotorSetSMCE()
{
    ...
    { // 减小访存开销,先保存指针引用
        ...
        //smc_ref = motor_controller->smc.smc_s; // 保存设定值,防止motor_controller->pid_ref在计算过程中被修改
        // 计算SMC控制律
        // if (motor_setting->motor_reverse_flag == MOTOR_DIRECTION_REVERSE)
        // {
        // motor_controller->smc_ref *= -1; // 设置反转
        // }
    
         if (motor->motor_can_instance->tx_id==5||motor->motor_can_instance->tx_id==6)
         {
           smc_out = SMC_Calc_1(&motor_controller->smc, measure->speed_aps,motor_controller->smc_ref) ;//+SMC_Calc_d(&motor_controller->smc,0,motor_controller->smc_diff);/* code */; /* code */
         }
         else if (motor->motor_can_instance->tx_id==7||motor->motor_can_instance->tx_id==8)
         {
            smc_out = SMC_Calc_2(&motor_controller->smc, measure->speed_aps,motor_controller->smc_ref);//+SMC_Calc_d(&motor_controller->smc, 0,motor_controller->smc_diff);/* code */
         }else
         {
            smc_out = 0;
         }
...
void DJIMotorControl()
{
    ...
    { // 减小访存开销,先保存指针引用
        ...
        if (motor_setting->feedback_reverse_flag == FEEDBACK_DIRECTION_REVERSE)
            ...
            if (measure->power_flag==1)
            {
                pid_ref=measure->vt_power;/* code */
                set = (int16_t)pid_ref;
            }
           else
           {

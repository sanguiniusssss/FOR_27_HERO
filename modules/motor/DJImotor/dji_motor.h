...
// 1. SMC参数配置（需根据实际系统整定，仅3个核心参数，调试比PID简单）

#define SMC_LAMBDA_1         0.5f     // 滑模面参数（决定收敛速度，小则慢，大则易抖）
#define SMC_K_1         10.0f    // 滑模增益（决定抗扰能力，足够大才能保证滑模运动）
#define SMC_beta_1        40.0f    // 滑模增益（决定抗扰能力，足够大才能保证滑模运动）
#define SMC_alpha_1     0.0001f     // 边界层厚度（抑制抖振，Δ越大抖振越小，过大会降低精度）
#define SMC_T         0.005f    // 控制周期（10ms，与系统采样周期一致，单位：s）
#define SMC_LAMBDA_2         0.5f     // 滑模面参数（决定收敛速度，小则慢，大则易抖）
#define SMC_K_2         5.0f    // 滑模增益（决定抗扰能力，足够大才能保证滑模运动）
#define SMC_beta_2        10.0f    // 滑模增益（决定抗扰能力，足够大才能保证滑模运动）
#define SMC_alpha_2     0.001f     // 边界层厚度（抑制抖振，Δ越大抖振越小，过大会降低精度）
#define SMC_LAMBDA_d         0.0f     // 滑模面参数（决定收敛速度，小则慢，大则易抖）
#define SMC_K_d         0.0f    // 滑模增益（决定抗扰能力，足够大才能保证滑模运动）
#define SMC_beta_d        0.0f    // 滑模增益（决定抗扰能力，足够大才能保证滑模运动）
#define SMC_alpha_d     0.0f     // 边界层厚度（抑制抖振，Δ越大抖振越小，过大会降低精度）
...
void SMCInit(SMC_Init_Config_s *smc);
static float smc_sat(float x);
float SMC_Calc(SMCInstance *smc,float actual_val,float ref);
...
void DJIMotor_Shoot_flont(uint8_t  shoot_flag);
